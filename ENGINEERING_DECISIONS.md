...
| D25 | Artifact compiler boundary and declared outcomes (Milestone 7) | `ArtifactCompiler` lives in `artifact/`, is a pure function (no I/O, no clock, no model) of the **persisted** `DISCOVERY_ENDED` record (`cua.evidence.events.DiscoveryEndedPayload`: verified stop reason + literal-free trace) and a `CapabilityDeclaration` (name, description, typed inputs/outputs, declared known outcomes); `cua.artifact.__init__` never imports it, so replay never loads it and the zero-model guard forbids it; the closed transforms moved to the leaf `cua.artifact.transforms` (replay re-exports) so the compiler shares them without loading the replay engine; one strategy per target — exactly the descriptor the discovery ambiguity guard resolved uniquely — no fallback, no `first()`; postconditions/checkpoint derived only from recorded routes and bindings (route-only checkpoint, never a heading the trace did not record); known business outcomes are declared beside the goal (`cua/discovery/capabilities.py`), labelled `DECLARED`, verified by replay; compiler output persisted to a separate store root `capabilities/generated/` with the same `name@version` (same contract and workflow; lineage = store root + `provenance.source`) plus `compile_reports/<id>.json` | `cua.discovery.__init__` eagerly loads `cua.llm`, so importing the trace type from `artifact/` would cycle and pull the model layer into replay; the persisted record is what a reviewer reads, so "what the compiler read" equals "what the reviewer can audit", and the official E01 exists only in that form; a successful trace can never evidence a business outcome (an outcome ends discovery as `REPORT_BLOCKED`), so declaration is the only authoritative source in V1 (D14: "declared at compile time"); refusing to invent verification semantics keeps the artifact honest to its evidence | The generated checkpoint is weaker than the handwritten bootstrap (route only); scoped descriptors are stricter than necessary (no minimization without an observation); `MEMBER_NOT_FOUND` is not discovered; a no-route-change CLICK gets a weak-but-true postcondition | A trace field or condition type for headings/titles exists; an outcome-discovery run merges `OutcomeCandidate` into `known_outcomes` labelled `DISCOVERED`; a registry keys artifacts on (source, name, version) |

D06 is retained verbatim as the historical record; D24 supersedes it for the implemented V1 provider
(Milestone 6, official E01 run `run_e49e4d0cbe09`). D25 records the Milestone 7 compiler boundary.
