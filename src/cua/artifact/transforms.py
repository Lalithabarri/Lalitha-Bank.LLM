...
Lives in ``artifact/`` because the vocabulary is the artifact's (``TransformType``) and both the
replay engine and the artifact compiler apply it: a pure leaf that imports nothing above the
schema, so importing it never loads the replay engine. ``cua.replay.transforms`` re-exports it
unchanged (Milestone 7; behaviour identical to Milestone 4).
"""

import re
from decimal import Decimal, InvalidOperation

from cua.artifact.schema import TransformType
